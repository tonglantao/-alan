...
Path_1 = r'D:\lbq\dataset\0_gjj\dataset\07_MultipleFluorescence\漫性大B细胞淋巴瘤\cityscapes\B细胞\gtFine'
Path_2 = r'D:\lbq\dataset\0_gjj\dataset\07_MultipleFluorescence\漫性大B细胞淋巴瘤\cityscapes\巨噬细胞\gtFine\train'
Path_3 = r'D:\lbq\dataset\0_gjj\dataset\07_MultipleFluorescence\漫性大B细胞淋巴瘤\cityscapes\细胞核\gtFine\train'
alise_path = r'D:\lbq\dataset\0_gjj\dataset\07_MultipleFluorescence\漫性大B细胞淋巴瘤\cityscapes\合成图\gtFine\train'

Path_1_img = glob.glob(os.path.join(Path_1, "*.png"))
Path_2_img = glob.glob(os.path.join(Path_2, "*.png"))
Path_3_img = glob.glob(os.path.join(Path_3, "*.png"))

for test_img in zip(Path_1_img,Path_2_img,Path_3_img):
    # test_img = 'specimen_02_tile_03_01_gtFine_labelIds.png'
    base_name = os.path.basename(test_img[0])
    foreground3 = np.array(Image.open(test_img[0]))
    background = np.array(Image.open(test_img[1]))
    foreground2 = np.array(Image.open(test_img[2]))

    synthetic_array = foreground3 + background + foreground2
    thre_array = np.where(synthetic_array > 0.5, 1, 0)
    img_tr = Image.fromarray(thre_array)
    bmp_P_web = img_tr.convert('P')

    # src_img = src_img.convert('RGB')
    # background.paste(foreground3, (0, 0), foreground3.convert('RGBA'))
    # background.paste(foreground2, (0, 0), foreground2.convert('RGBA'))
    dst_name = os.path.join(alise_path,base_name)
    bmp_P_web.save(dst_name)

# bmp_P_web = src_img.convert('P')
# web_palette = bmp_P_web.getpalette()  # <---
# data = np.array(bmp_P_web)
# # second part
# bmp = Image.fromarray(data)
#
# bmp.putpalette(LABEL_MAP)
# bmp.show()
